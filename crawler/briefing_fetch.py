...
CONN = ('가운데', '대응하여', '대응해', '따라', '속에서', '맞춰', '대비해', '와중에', '추세에',
        '위해', '위하여', '불구하고', '있어', '넘어', '함에 따라', '더불어', '가운데서')


def strip_opinion(t, cat=None):
    """제언·해석·타사 훈수 절을 제거하고 사실만 남김. 살릴 게 없으면 None(→ 호출부가 기사 스니펫으로 대체)."""
    t = str(t or '').strip()
    if not t:
        return None
    clauses = [c.strip() for c in re.split(r',\s*', t)]

    def bad(c):
        if any(b in c for b in BAN):
            return True
        if cat and cat != '자사' and '아카라' in c:  # 경쟁사/시장/업계 insight에 아카라 훈수 제거
            return True
        return False
    kept = [c for c in clauses if not bad(c)]
    # 제언을 이끌던 연결어만 남은 후행 절 제거
    while kept and any(kept[-1].rstrip().rstrip('.').endswith(x) for x in CONN):
        kept.pop()
    out = ', '.join(kept).strip().strip(',').strip()
    if len(out) < 10:
        return None
    if not re.search(r'[.?!다함음됨됐다]$', out):
        out += '.'
    return out
...
def main():
    ...
    for p in (res.get("rows", []) or []):
        ...
        if not it:
            continue
        cat = p.get("cat", "시장")
        ins = strip_opinion(p.get("insight", ""), cat) or (it.get("snippet") or "").strip() or p.get("insight", "")
    ...
    hl = {k: (strip_opinion(v, k) or v) for k, v in hl.items()}
