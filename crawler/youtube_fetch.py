...
STOP = set(("그냥 진짜 정말 너무 이거 저거 근데 그리고 하는 있는 저는 저도 제가 나도 영상 그거 이제 요즘 하고 보고 같아요 있어요 네요 어요 아요 에서 합니다 하네요 이런 저런 그런 으로 인데 는데 스마트 아카라 완전 약간 조금 많이 때문 하지만 지금 오늘 정도 사람 우리 저희 그때 "
             "감사합니다 감사 항상 싶네 좋네 좋아요 같아 있는데 이번 엄청 있습니다 그램 하나 되게 그건 이건 어떻게 아니 그래서 그러 저러 모카 강나미 지니 앱준 판다 니까 는거 하니 보니 라서 는지 어요 습니다 세요 네여 " ).split())
        ...
        for _ in range(3):
            ...
            for c in cr.get("items", []):
                ...
                if hp and not hn:
                    ...
                    n += 1
                tk = re.sub(r"https?://\S+|www\.\S+|[A-Za-z]+\.\S+", " ", t)
                for w in re.split(r"[^0-9A-Za-z가-힣]+", tk):
                    if len(w) < 2:
                        continue
                    w = JOSA.sub("", w)
                    if len(w) < 2 or w in STOP:
                        continue
                    if re.fullmatch(r"[A-Za-z]{1,3}", w):  # EC, utm 등 짧은 영문 노이즈 제거
                        continue
